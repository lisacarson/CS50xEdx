...
function addMarker(place)
{
  ...
  var marker = new MarkerWithLabel({
    icon: "https://maps.google.com/mapfiles/kml/pal3/icon63.png", // http://www.lass.it/Web/viewer.aspx?id=4
    position: myLatLng,
    map: map,
    title: 'Hello World!'
  });
  ...
  google.maps.event.addListener(marker, "click", function() {
    ...
    {
        var list = "<ul>";
        var listTemplate = _.template("<li><a href= '<%- link %>' target='_blank'><%- title %></a></li>");
        for (var i = 0; i < data.length; i++)
        {
            list += listTemplate({link: data[i].link, title: data[i].title});
        }
        list += "</ul>";
