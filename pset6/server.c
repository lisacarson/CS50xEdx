...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char absolute_path[length];
            char * absolute = strtok(back, "?");
            if (absolute == NULL)
                strcpy(absolute_path, absolute);
                
            else
                strcpy(absolute_path, back);
            ...
            char* pre = strchr(absolute_path, '.') + 1;
